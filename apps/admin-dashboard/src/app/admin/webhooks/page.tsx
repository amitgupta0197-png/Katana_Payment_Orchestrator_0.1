      ...
      <Tabs defaultValue="configs">
        ...
        <TabsContent value="configs">
          <DataView rows={configs} columns={configCols} rowKey={(r) => r.config_id} loading={q.isLoading}
            search={{ placeholder: "Search by banker id or URL…", fields: ["merchant_id", "target_url"] }}
            filters={[
            ...
            emptyDescription="Click Create Webhook to subscribe a banker URL to platform events."
            rowActions={(r) => (
              <RowActions
                actions={[
        ...
        <TabsContent value="pending">
          <DataView rows={pending} columns={pendingCols} rowKey={(r) => r.outbox_id} loading={q.isLoading}
            search={{ placeholder: "Search by event / banker…", fields: ["event_type", "merchant_id", "target_url"] }}
            savedViewKey="webhooks-pending"
            emptyTitle="Queue is empty" emptyDescription="Trigger any banker-bound event to enqueue."
            rowActions={(r) => (
              <RowActions actions={[
