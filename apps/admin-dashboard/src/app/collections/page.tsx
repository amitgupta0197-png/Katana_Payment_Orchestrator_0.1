...
export default function CollectionsPage() {
  ...
  const cols: Column<VA>[] = [
    { key: "counterparty", header: "Banker" },
  ...
  return (
    <>
      <PageHeader title="Collections (VA)" description="Virtual-account mappings per banker." icon={Inbox} />
      <DataView rows={rows} columns={cols} rowKey={(r) => r.id} loading={q.isLoading}
        search={{ placeholder: "Search by banker / VA / IFSC / VPA…", fields: ["counterparty", "va_account_no", "va_ifsc", "va_upi_vpa"] }}
        filters={[
