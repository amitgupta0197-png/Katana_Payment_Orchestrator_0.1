...
export default function MerchantWalletPage() {
  ...
  return (
    <>
      <PageHeader title="Banker wallet" description="Per-branch funded balance, derived from journal_entries." icon={Wallet} />
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiTile label="Bankers" value={new Set(rows.map((r) => r.merchant_id)).size} loading={q.isLoading} />
      ...
      <DataView rows={rows} columns={cols} rowKey={(r) => `${r.merchant_id}-${r.currency}`} loading={q.isLoading}
        search={{ placeholder: "Search by banker…", fields: ["merchant_id", "currency"] }}
