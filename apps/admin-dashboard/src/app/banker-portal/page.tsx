...
export default function MerchantDashboard() {
  ...
  if (todayFailed > 0 && (successRate ?? 100) < 95) {
    alerts.push({ level: "critical", title: `Success rate ${successRate}% today`, detail: `${todayFailed} of ${todayOrders.length} payments failed.`, href: "/banker-portal/transactions?f=failed", cta: "Inspect" });
  }
  if (openDisputes > 0) {
    alerts.push({ level: "warning", title: `${openDisputes} open dispute${openDisputes > 1 ? "s" : ""}`, detail: "Reply window may be running out — SLA clock applies.", href: "/banker-portal/disputes", cta: "Reply" });
  }
  if (releasingSoon > 0) {
    alerts.push({ level: "info", title: `${releasingSoon} reserve release${releasingSoon > 1 ? "s" : ""} in next 7 days`, href: "/banker-portal/reserves", cta: "View calendar" });
  }
  ...
  return (
    <>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <KpiTile label="Open disputes" value={openDisputes} icon={ShieldAlert} variant={openDisputes > 0 ? "warning" : "default"} loading={disputes.isLoading} href="/banker-portal/disputes" />
      </div>

      <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-[color:var(--color-text-muted)]">Money</h2>
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiTile label="Reserves held" value={formatAmount(heldNow)} sublabel={`${reserveRows.length} schedules`} icon={Banknote} loading={reserves.isLoading} href="/banker-portal/reserves" />
        <KpiTile label="Releasing in 7d" value={releasingSoon} icon={Banknote} loading={reserves.isLoading} href="/banker-portal/reserves" />
        <KpiTile label="Pending settlement" value={formatAmount(currentBalance)} sublabel="next batch ETA" icon={Receipt} loading={balance.isLoading} href="/banker-portal/settlements" />
        <KpiTile label="Pay-in count today" value={todayOrders.length} icon={Activity} loading={txnsLoading} href="/banker-portal/transactions" />
      </div>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-2">
            <Button variant="secondary" asChild className="justify-between">
              <Link href="/banker-portal/api-keys"><span className="inline-flex items-center gap-2"><KeyRound className="h-4 w-4" />Issue API key</span><ChevronRight className="h-3.5 w-3.5" /></Link>
            </Button>
            <Button variant="secondary" asChild className="justify-between">
              <Link href="/banker-portal/profile"><span className="inline-flex items-center gap-2"><Webhook className="h-4 w-4" />Configure webhooks</span><ChevronRight className="h-3.5 w-3.5" /></Link>
            </Button>
            <Button variant="secondary" asChild className="justify-between">
              <Link href="/banker-portal/sub-mids"><span className="inline-flex items-center gap-2"><Plus className="h-4 w-4" />Request a new Sub-MID</span><ChevronRight className="h-3.5 w-3.5" /></Link>
            </Button>
            <Button variant="secondary" asChild className="justify-between">
              <Link href="/banker-portal/settlements"><span className="inline-flex items-center gap-2"><Receipt className="h-4 w-4" />Settlement statement</span><ChevronRight className="h-3.5 w-3.5" /></Link>
            </Button>
          </CardContent>
        </Card>
      </div>
