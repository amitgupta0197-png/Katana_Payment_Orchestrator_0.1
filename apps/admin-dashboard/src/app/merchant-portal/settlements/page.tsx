...
function RaiseDialog({ open, onOpenChange, providerId, branches, beneficiaries }: { open: boolean; onOpenChange: (o: boolean) => void; providerId: string; branches: MerchantRow[]; beneficiaries: Benef[] }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Raise settlement to a banker</DialogTitle><DialogDescription>Amount is prefilled from the banker's outstanding collected pay-ins — edit if needed.</DialogDescription></DialogHeader>
        <div className="space-y-3">
          ...
          {branch && branchCap && (
            <div className="rounded-md border bg-[color:var(--color-surface-muted)] p-2 text-xs text-[color:var(--color-text-muted)]">
              Banker declares: bank {branchCap.bank_available ? "✓" : "✗"} · USDT {branchCap.usdt_available ? `✓ ${branchCap.usdt_quantity ?? "?"} on ${branchCap.usdt_network ?? "?"}` : "✗"}
