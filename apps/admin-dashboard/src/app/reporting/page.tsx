...
export default function ReportingPage() {
  ...
  return (
    <>
      <PageHeader title="Reporting" description="Daily banker roll-ups + recent transaction facts (PRODUCT_VISION §3.11)." icon={BarChart3} />
      ...
      <Tabs defaultValue="daily">
        ...
        <TabsContent value="daily">
          <DataView rows={daily} columns={dCols} rowKey={(r) => `${r.merchant_id}|${r.day}|${r.kind}|${r.status}|${r.currency}`} loading={q.isLoading}
            search={{ placeholder: "Search by banker / kind…", fields: ["merchant_id", "kind", "status"] }}
        ...
        <TabsContent value="facts">
          <DataView rows={facts} columns={fCols} rowKey={(r) => r.id}
            search={{ placeholder: "Search by banker / rail / kind…", fields: ["merchant_id", "kind", "rail", "method"] }}
