...
async function readConfig(code: string) {
  const r = await rows<any>("merchant",
    `SELECT enabled_methods, poolpay FROM merchant_payment_config WHERE merchant_code = $1`, [code]);
  if (!r.length) return { enabled_methods: DEFAULT_METHODS, poolpay: { enabled: false } as Record<string, unknown> };
  return { enabled_methods: r[0].enabled_methods ?? DEFAULT_METHODS, poolpay: r[0].poolpay ?? {} };
}
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await rows("merchant", `
      INSERT INTO merchant_payment_config (merchant_code, enabled_methods, poolpay, updated_by, updated_at)
      VALUES ($1, $2::jsonb, $3::jsonb, $4, now())
      ON CONFLICT (merchant_code) DO UPDATE
        SET enabled_methods = EXCLUDED.enabled_methods, poolpay = EXCLUDED.poolpay,
            updated_by = EXCLUDED.updated_by, updated_at = now()
    `, [scope.code, JSON.stringify(enabled_methods), JSON.stringify(poolpay), g.session.email]);
    return NextResponse.json({ methods: METHODS, enabled_methods, poolpay });
  } catch (err) { const e = pgError(err); return NextResponse.json(e.body, { status: e.status }); }
}
