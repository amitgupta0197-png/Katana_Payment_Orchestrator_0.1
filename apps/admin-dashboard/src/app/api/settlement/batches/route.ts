...
export async function GET() {
  ...
  try {
    ...
    // Columns match the canonical settlement_batches schema; net_amount is
    // surfaced as net_payable for the /settlement page.
    const batches = await rows<any>("settlement", `
      SELECT id, tenant_id, merchant_id,
             batch_date, period_start, period_end, currency,
             gross_amount, fee_amount, reserve_amount,
