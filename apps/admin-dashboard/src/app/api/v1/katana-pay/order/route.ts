// POST /api/v1/katana-pay/order — merchant-facing pay-in (S2S) order creation,
// authenticated by the merchant's Katana checkout Key + Salt (NOT a session;
// allow-listed in middleware). This is the production path: a merchant's (or
// provider's) server signs and calls this to create a pay-in; the order is then
// confirmed by the trusted device's bank-credit SMS, a webhook, or ops (UTR).
//
// Signature is the SAME scheme used for /api/pay:
