...
export default function FifoSettlementsPage() {
  ...
  return (
    <>
      ...
      <Card className="mb-4">
        <CardHeader><CardTitle className="text-base">Create settlement batch</CardTitle><CardDescription>Nets all COMPLETED, unsettled pay-ins for the banker. Optional chargeback hold / approved adjustment (₹).</CardDescription></CardHeader>
        <CardContent className="flex flex-wrap items-end gap-2">
          <Input className="h-9 w-48" placeholder="banker code e.g. M10001" value={f.merchant_id} onChange={(e) => setF({ ...f, merchant_id: e.target.value })} />
      ...
      <Card>
        ...
        <CardContent className="overflow-x-auto">
          {batches.length === 0 ? <EmptyState icon={Banknote} title="No settlement batches yet" description="Enter a banker code above and create a batch to net their completed pay-ins into a payout. Large or adjusted batches go to maker-checker." /> : (
            <table className="w-full text-xs">
              <thead><tr className="border-b text-left text-[color:var(--color-text-muted)]">
                <th className="px-2 py-1.5">batch</th><th className="px-2 py-1.5">banker</th><th className="px-2 py-1.5">orders</th>
